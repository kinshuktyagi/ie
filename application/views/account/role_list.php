	...
	<div class="content">
		...
		<div class="row">
			  <div class="col-sm-12 text-right">
				<a href="<?php echo base_url("account/role/add"); ?>"><button type="button" class="btn btn-primary w-md m-rb-5">Add Role</button></a>
				<a href="<?php echo base_url("account/role/reset_filter"); ?>"><button type="button" class="btn btn-warning w-md m-rb-5">Reset Filter</button></a>
			  </div>
		</div>
		<?php 
		/* echo "<pre>";
		...
		<div class="row">
			  <div class="col-sm-12">
				<div class="panel lobidisable panel-bd">
					...
					<form method="POST" action="<?php echo base_url("account/role/index"); ?>">
						...
						<div class="table-responsive">
							<table class="table table-bordered">
								<thead>
									...
									<tr>
										<td><input type="text" name="role_name" placeholder="Role Name" value="<?php if(isset($_SESSION['search']['role_name'])){ echo $_SESSION['search']['role_name']; } ?>" class="form-control"/></td>
											...
											for($i=0;$i<sizeof($data);$i++)
											{
												?>
													<tr>
														...
														<td>
															<select id="<?php echo $data[$i]['role_id']; ?>" class="action">
		...
		<script>
			...
			$(".action").change(function()
			{
				...
				if(action=='Disable')
				{
					document.location="<?php echo base_url("account/role/disable?role_id=") ?>"+id;
				}
				if(action=='Enable')
				{
					document.location="<?php echo base_url("account/role/enable?role_id=") ?>"+id;
				}
				if(action=='Manage')
				{
					document.location="<?php echo base_url("account/role/manage?role_id=") ?>"+id;
				}
			});
